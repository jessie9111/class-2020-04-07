...
```{r scene 2}
x %>%
  summarize(cor_salary_studincome = cor(faculty, earnings)) 

x %>%
  summarize(cor_sat_studincome = cor(sat, earnings))
...
school_model <- lm(earnings ~ sat + faculty, data = x)
...
```
The Intercept estimate is the value when the SAT score and faculty earnings are equal to 0.

For every 1 point increase in SAT score, student earnings should increase by about 24.48 units.

For every 1 unit increase in faculty earnings, student earnings should increase by about 0.38 units.
...
Treatment: Cutting the financial aid budget
Outcomes: Earnings of students 10 years from now

Fundamental problem of causal inference: we cannot know both the outcomes when we cut the budget and when we do not

We would have to assume randomized treatment to make a causal claim.
...
sat_estimate = 24.4804028	
faculty_estimate = 0.3853723	
intercept = -2590.9829154
